...
const getAllSertifikasi = async (req, res) => {
    try {
        const sertifikasi = await prisma.sertifikasi.findMany({
            include: { peserta : true, pelatihan : true } //sertakan data peserta
        });
...
const getSertifikasiById = async (req, res) => {
    try {
        ...
        const sertifikasi = await prisma.sertifikasi.findUnique({
            where: { id },
            include : { peserta : true, pelatihan : true }
        });
    
    if (!sertifikasi) return res.status(404).json({message: 'Certificate not found'});
    return res.json(sertifikasi);
    } catch (error) {
        console.error(error);
        return res.status(500).json({message : 'Internal Server Error'});
...
const createSertifikasi = async (req, res) => {
    try {
        const { Nama_dokumen, tanggal_dan_bulan, id_peserta, id_pelatihan } = req.body;
...
const updateSertifikasi = async (req, res) => {
    try {
      ...
      data.tanggal_dan_bulan = date;
        ...
        console.error(error);
        if (error.code === 'P2025') { //P2025 itu error code nya prisma, kalau hasilnya record not found
