...
export default function FormPeserta() {
  ...
  // Ambil data user & pelatihan
  const fetchPelatihan = () => {
    fetch("http://localhost:3000/api/Pelatihan")
      .then((res) => res.json())
      .then((data) => setPelatihans(data))
  ...
  const handleRemovePeserta = (index) => {
    const updatedList = pesertaList.filter((_, i) => i !== index);
    setPesertaList(updatedList);
  };
  ...
  // Submit semua peserta sekaligus
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Backend expect "peserta" bukan "pesertaList"
      formData.append("peserta", JSON.stringify(pesertaList));
      formData.append("id_user", id_User);
      formData.append("id_pelatihan", id_Pelatihan);
      // formData.append("id_bidang", ""); // Backend butuh ini, tapi bisa kosong
      ...
      const response = await fetch("http://localhost:3000/api/Peserta/bulk", {
        method: "POST",
        body: formData,
      });
    ...
    <div className="d-flex align-items-center justify-content-center min-vh-100 bg-light py-5">
      <div className="container d-flex shadow-lg rounded-4 overflow-hidden bg-white" style={{ maxWidth: "1100px" }}>
        ...
        <div
          ...
          style={{
            background:
              "linear-gradient(135deg, rgba(35, 27, 83, 1), rgba(92, 0, 69, 0.39))",
          }}
        >
          <h1 className="fw-bold mb-3">Daftar Peserta</h1>
          <p className="medium">
            Tambahkan beberapa peserta sekaligus untuk pelatihan Anda di sini.
          </p>
        </div>
        ...
        <div className="w-50 d-flex flex-column justify-content-start bg-white p-4" style={{ maxHeight: "90vh", overflowY: "auto" }}>
          <div className="w-100 px-3">
            ...
            {/* Upload File - SATU UNTUK SEMUA PESERTA */}
            <div className="mb-3">
              ...
              />
              {uploadedFile && (
                <small className="text-muted">File: {uploadedFile.name}</small>
              )}
            </div>
